...
const ManagerSwitcher = ({ className, items = defaultItems, handleChange = () => {} }) => {
  ...
  return (
    <Box sx={{
        display: 'flex', flexDirection:{sm: 'column', md:'row'}, flexWrap: 'wrap',  justifyContent:'space-between', backGroundColor:'purple'
        }} >
      ...
      {items.map(({title,id}, i) => {
        return (
          ...
          <Typography
            ...
            key={id+i}
            sx={{color:'green', cursor:'pointer', padding:'0.5rem', textAlign:'center', border:'1px solid grey', minWidth:'100px', borderRadius:'8px', }}
